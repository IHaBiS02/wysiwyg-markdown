...
- WYSIWYG 블록을 더블클릭하면 해당 블록을 Markdown 소스로 수정할 수 있다.
...
- WYSIWYG, 블록 소스 편집, 전체 소스 편집, readonly 모드를 한 화면에서 확인할 수 있어야 한다.
...
- 적용 시 파싱한 결과가 기존 블록을 대체한다.
- 적용 전체를 하나의 undo transaction으로 기록한다.
- 파싱에 실패하면 기존 내용을 유지하고 오류를 표시한다.

### 7.3 전체 문서 소스 모드

- 공개 API 또는 툴바 버튼으로 전환한다.
...
- 블록 더블클릭으로 Markdown 소스를 수정할 수 있다.
